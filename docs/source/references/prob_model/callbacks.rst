Training Callbacks
===============================
This section describes :class:`~fortuna.training.callback.Callback` that allows users to add custom actions at different stages of the training loop.

In order to train a :class:`~fortuna.prob_model.base.ProbModel` using callbacks the user has to:

- Define their own callbacks by subclassing :class:`~fortuna.training.callback.Callback` and override the methods of interest.
...
The following example outlines the usage of :class:`~fortuna.training.callback.Callback`.
