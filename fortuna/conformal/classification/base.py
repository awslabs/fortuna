...
class ConformalClassifier:
    ...
    def _get_conformal_sets_from_scores(
        ...
        conds = jnp.sum(val_scores[:, None, None] > test_scores[None], axis=0) < (1 - error) * (len(val_scores) + 1)
        sizes = conds.sum(1)
...
class SplitConformalClassifier(ConformalClassifier, abc.ABC):
    def get_scores(
        ...
        test_scores = vmap(
            lambda i: self.score_fn(test_probs, i * jnp.ones(len(test_probs), dtype="int32")), out_axes=1
    ...
    def conformal_set(
        ...
        val_scores, test_scores = self.get_scores(val_probs=val_probs, val_targets=val_targets, test_probs=test_probs)
        return super()._get_conformal_sets_from_scores(val_scores=val_scores, test_scores=test_scores, error=error)
...
class CVPlusConformalClassifier(ConformalClassifier):
    def conformal_set(
        ...
        val_scores, test_scores = self.get_scores(cross_val_probs=cross_val_probs, cross_val_targets=cross_val_targets, cross_test_probs=cross_test_probs)
        return super()._get_conformal_sets_from_scores(val_scores=val_scores, test_scores=test_scores, error=error)
    ...
    def get_scores(
        ...
        for val_probs, val_targets, test_probs in zip(cross_val_probs, cross_val_targets, cross_test_probs):
