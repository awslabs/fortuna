...
from typing import Iterable, Union, Tuple, Optional, Any, Callable

import numpy as np

from fortuna.typing import Batch, InputData, Targets, Array
...
class IterableData:
    ...
    def inputs_loaders_to_batch_iterable(cls, inputs_loaders, targets) -> IterableData:
        def _inner():
            for all_inputs in zip_longest(*inputs_loaders):
                _targets = [target * np.ones(inputs.shape[0], dtype="int32") for inputs, target in zip(all_inputs, targets) if inputs is not None]
    ...
    @classmethod
    def transform_inputs_or_targets_loader(cls, loader, transform_fn, status) -> IterableData:
        def _inner():
            _status = deepcopy(status)
            for data in loader:
