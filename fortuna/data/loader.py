...
class DataLoader:
    ...
    @classmethod
    def from_inputs_loader(cls, inputs_loaders: List[InputsLoader], targets: List[int]) -> DataLoader:
        """
...
class FromInputsLoadersToDataLoader:
    ...
    def __call__(self):
        for all_inputs in zip_longest(*self._inputs_loaders):
            all_inputs = [inputs if inputs is not None else np.array([]) for inputs in all_inputs]
            targets = [target * np.ones(inputs.shape[0], dtype="int32") for inputs, target in zip(all_inputs, self._targets)]
