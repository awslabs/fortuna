...
from fortuna.prob_model.posterior.sgmcmc.hmc.hmc_trainer import (
    HMCTrainer,
    JittedHMCTrainer,
    MultiDeviceHMCTrainer,
)
...
from fortuna.prob_model.posterior.sgmcmc.hmc.hmc_callback import (
    HMCSamplingCallback,
)
from fortuna.prob_model.posterior.sgmcmc.hmc.hmc_integrator import (
    hmc_integrator,
)
...
class HMCPosterior(SGMCMCPosterior):
    def __init__(
        self,
        joint: Joint,
        posterior_approximator: HMCPosteriorApproximator,
    ):
        """
        Hamiltonian Monte Carlo approximate posterior class.
    ...
    def fit(
        ...
        **kwargs,
        ...
        if fit_config.optimizer.method is not None:
            logging.info(f"`FitOptimizer` method in HMC is ignored.")

        fit_config.optimizer.method = hmc_integrator(
            integration_steps=self.posterior_approximator.integration_steps,
        ...
        trainer_cls = select_trainer_given_devices(
            ...
            base_trainer_cls=HMCTrainer,
            jitted_trainer_cls=JittedHMCTrainer,
            multi_device_trainer_cls=MultiDeviceHMCTrainer,
        ...
        state = HMCState.convert_from_map_state(
            map_state=state,
        ...
        hmc_sampling_callback = HMCSamplingCallback(
        ...
        logging.info(f"Run HMC.")
        state, status["hmc"] = trainer.train(
            rng=self.rng.get(),
            state=state,
            ...
            callbacks=[hmc_sampling_callback],
        )
